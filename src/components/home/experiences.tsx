...
export default function Experiences() {
    ...
    return <section className={'px-16 my-24 bg-white'}>
           ...
           <ExperienceCard experience={experiences[0]}/>
            <div className={'text-end flex flex-col gap-4'}>
                ...
                <div className={'flex flex-row gap-4 ms-auto w-2/3 justify-end flex-wrap'}>
                    {experiencesTypes.map((experience,index) => <Pill key={index} text={experience} active={index === currentIndex} onClick={() => setCurrentIndex(index)}/>)}
                </div>
            </div>
            {experiences.slice(1).map((experience, index) => <ExperienceCard experience={experience} key={index}/>)}
        </div>
    </section>
}
...
function Pill(this: any, {text, active, onClick}: { text: string, active: boolean, onClick?: () => void }) {

    return <div
        className={`${active ? 'bg-dark text-white' : 'bg-white text-dark'} rounded-full py-2 px-3 text-nowrap border border-gray-500 active:bg-dark active:text-white hover:bg-dark hover:text-white`}
